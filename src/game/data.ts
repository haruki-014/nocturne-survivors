...
export function describeWeaponUpgrade(def: WeaponDef, lv: number): string {
  ...
  if (b.cooldown < a.cooldown)
    parts.push(`間隔 -${r2(a.cooldown - b.cooldown)}秒`);
  if (b.area > a.area)
    parts.push(`範囲 +${Math.round((b.area / a.area - 1) * 100)}%`);
  if (b.pierce > a.pierce && b.pierce < 900)
    parts.push(`貫通 +${b.pierce - a.pierce}`);
  if (b.speed > a.speed && def.id !== "orbs")
    parts.push(`弾速 +${Math.round(b.speed - a.speed)}`);
  ...
  grimoire: [
    {
      req: "tome",
      evo: "grimoire_codex",
      desc: "詠唱が途切れぬ。魔弾は奔流となり、貫いて止まらない。",
    },
    {
      req: "might",
      evo: "grimoire_blasphemy",
      desc: "禁忌の一節。少数の弾が、すべてを撃ち抜く巨弾と化す。",
    },
  ],
  knife: [
    {
  ...
  orbs: [
    {
      req: "candle",
      evo: "orbs_halo",
      desc: "月光が環を拡げる。聖環が高速で旋回し、触れる者を灼く。",
    },
  ],
  censer: [
    {
      req: "heart",
      evo: "censer_sanctuary",
      desc: "鼓動が炎を育てる。広大な聖域が不浄を焼き続ける。",
    },
  ],
  lightning: [
    {
...
export const schoolTier = (count: number): number =>
  count >= 5 ? 2 : count >= 3 ? 1 : 0;
...
export const PASSIVES: Record<string, PassiveDef> = {
  ...
  regen: {
    ...
    maxLevel: 5,
    levelDesc: (lv) =>
      `毎秒 +0.45 HP 再生（合計 +${(0.45 * lv).toFixed(2)}/秒）`,
  },
  duplicator: {
    id: "duplicator",
    name: "写し身の鏡",
...
export const BOSSES: BossDef[] = [
  ...
  },
  {
    ...
    radiusMul: 1.08,
    ability: "rally",
    trait: "夜の眷属を鼓舞し、火炎弾を撒き散らす",
  },
];

export const BOSSES_BY_ID: Record<string, BossDef> = Object.fromEntries(
  BOSSES.map((b) => [b.id, b]),
);
...
export function weaponStatBadges(
  def: WeaponDef,
  lv: number,
): { label: string; value: string }[] {
  const s = def.statsFor(lv);
  const out: { label: string; value: string }[] = [
    { label: "威力", value: String(Math.round(s.damage)) },
  ];
  if (def.behavior !== "aura")
    out.push({ label: "数", value: String(s.amount) });
  out.push({
    label: "間隔",
    value: `${(Math.round(s.cooldown * 100) / 100).toFixed(2)}s`,
  });
  return out;
}
...
// ------------------------------------------------------------
export const REGULAR_KINDS: EnemyKind[] = [
  "bat",
  "zombie",
  "skeleton",
  "wraith",
...
export const SKINS: SkinDef[] = [
  ...
  },
  {
    id: "crimson",
    name: "緋の伯爵狩り",
    desc: "返り血の色を纏う、討伐者の証。【専用技：緋月の戦鎌】",
    cloak: "#3a1020",
    rim: "#8a2230",
    scarf: "#e0455e",
    face: "#efe0d2",
    lantern: "#ff6f7e",
    signature: "crimson_scythe",
  },
  {
    id: "verdant",
    name: "月光の祭司",
    desc: "聖域の緑を映す、祈りの法衣。【専用技：聖域の薫光】",
    cloak: "#163228",
    rim: "#3f7f6f",
    scarf: "#7be08a",
    face: "#e6ddc8",
    lantern: "#a8e6b0",
    signature: "verdant_bloom",
  },
  {
    id: "ember",
    name: "焔の巡礼",
    desc: "灰の下にくすぶる熾火を宿す装い。【専用技：業火の輪舞】",
    cloak: "#341812",
    rim: "#7e3a22",
    scarf: "#ff7a3c",
    face: "#efdcc6",
    lantern: "#ff9a4e",
    signature: "ember_waltz",
  },
  {
    id: "frost",
    name: "霜夜の狩人",
    desc: "凍てつく夜気を切り裂く白の狩衣。【専用技：氷牙の連弾】",
    cloak: "#16222e",
    rim: "#3f6f8f",
    scarf: "#8fe6ff",
    face: "#e6eef2",
    lantern: "#bfe9ff",
    signature: "frost_lance",
  },
  {
    id: "plague",
    name: "疫病の医師",
    desc: "腐臭の只中を歩む、鴉面の医師。【専用技：疫癘の散弾】",
    cloak: "#1c2418",
    rim: "#4f6a3f",
    scarf: "#b6d27a",
    face: "#cdd6c0",
    lantern: "#cfe08a",
    signature: "plague_fan",
  },
  {
    id: "royal",
    name: "月の王",
    desc: "夜を統べる者にのみ許される紫紺の式装。【専用技：王権の雷霆】",
    cloak: "#1d1630",
    rim: "#7a5cc0",
    scarf: "#c8a8ff",
    face: "#efe6ff",
    lantern: "#e0c8ff",
    signature: "royal_thunder",
  },
  {
    id: "gold",
    name: "黄金詠唱者",
    desc: "勝利を重ねた者に灯る黄金の威光。【専用技：黄金の聖句】",
    cloak: "#2a2410",
    rim: "#a9791f",
    scarf: "#ffe28a",
    face: "#f0e6cf",
    lantern: "#ffd66a",
    signature: "gold_verse",
  },
  {
    id: "void",
    name: "虚無の影",
    desc: "真化の極みに触れた者が沈む紫闇。【専用技：虚無の鎖環】",
    cloak: "#14121c",
    rim: "#3a3152",
    scarf: "#b078ff",
    face: "#d8cfe6",
    lantern: "#c69bff",
    signature: "void_chain",
  },
];

export const SKINS_BY_ID: Record<string, SkinDef> = Object.fromEntries(
  SKINS.map((s) => [s.id, s]),
);
...
export const CURIOS_BY_ID: Record<string, CurioDef> = Object.fromEntries(
  CURIOS.map((c) => [c.id, c]),
);
